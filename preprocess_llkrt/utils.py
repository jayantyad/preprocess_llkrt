...
def _get_stopwords_counts(x):
	l = len([w for w in x.split() if w in stopwords])
	return l
...
def _get_digit_counts(x):
	l = len([t for t in x.split() if t.isdigit() ==True])
	return l

def _get_uppercase_counts(x):
	return len([t for t in x.split() if t.isupper() ==True])
...
def _remove_common_words(x, n=20):
	text = x.split()
	freq_comm = pd.Series(text.split()).value_counts()
	fn = freq_comm[:n]
